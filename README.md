...
- [Использование ядра](#использование-ядра)
  ...
  - [Рабочие файлы ядра и обработка списков](#рабочие-файлы-ядра-и-обработка-списков)
    - [Логика работы](#логика-работы)
    - [Файлы сохраняемые ядром в папке данных](#файлы-сохраняемые-ядром-в-папке-данных)
...
- [Прочее](#прочее)
  - [Собери свою Запретян](#собери-свою-запретян)
    ...
    - [Шаг 1. Загрузка исходного кода](#шаг-1-загрузка-исходного-кода)
      - [Вариант 1: Загрузка актуального кода (Dev)](#вариант-1-загрузка-актуального-кода-dev)
      - [Вариант 2: Клонирование репозитория через Git (Альтернатива)](#вариант-2-клонирование-репозитория-через-git-альтернатива)
      - [Вариант 3: Загрузка последнего стабильного релиза](#вариант-3-загрузка-последнего-стабильного-релиза)
    - [Шаг 2. Сборка основного приложения (Ядро)](#шаг-2-сборка-основного-приложения-ядро)
    - [Шаг 3. Сборка расширений (Extensions)](#шаг-3-сборка-расширений-extensions)
      - [Вариант 1: Файл `main.go` находится в корне расширения](#вариант-1-файл-maingo-находится-в-корне-расширения)
      - [Вариант 2: Расширение содержит подпапку `cmd`](#вариант-2-расширение-содержит-подпапку-cmd)
    - [Итоговая структура рабочей папки](#итоговая-структура-рабочей-папки)
...
> - Для списков `domain_source` и `community_domain_sources` поддерживаются записи домена (example.com) и ссылки (http://example.com). Домен будет автоматически извлечён если в списке оказалась ссылка.
> - Для списков `ip_source` поддерживаются записи IP адреса (1.1.1.1) и CIDR (1.1.1.0/24). CIDR будут автоматически распакованы в полный список адресов этой подсети. 
> - Работа с IPv6 возможна, но не проверена и изначально не была предусмотрена.
...
- **`name` (Обязательное) (УНИКАЛЬНОЕ)**
  - Поле для имени плагина заполняется как `name = "my plugin"`. Это имя будет использовано при инициализации и при трансляции логов из плагина через ядро. Вы можете устанавлиать любое имя, но оно не должно использоваться другим плагином, иначе ядро завершит работу на стадии инициализации плагинов.
- `desc` (Необязательное)
  - Нужно для более простой навигации пользователя по конфигурации при поиске вашего плагина. Этот параметр будет передан плагину, но не учитывается ядром. Заполняется как `desc = "A plugin created just for being here"`
...
> 
> [Подробнее...](https://github.com/SHULKERPLAY/Zapretyan-Go/tree/main/Lab/Examples#%D0%BA%D0%BE%D0%BD%D1%81%D0%BE%D0%BB%D1%8C-%D0%BA%D0%B0%D0%BA-%D0%BF%D1%80%D0%BE%D0%BA%D1%81%D0%B8-%D0%B4%D0%BB%D1%8F-linux)
...
## Рабочие файлы ядра и обработка списков
### Логика работы
Все файлы списков, загруженные из массива источников, которые указаны в конфигурации, сливаются в один список, сортируются по алфавиту, чистятся от дубликатов и после этого записываются в конечный файл.

- Файлы, загруженные из `domain_source = ['https://...','https://...']` сохраняются в `new.tmp`
- Файлы, загруженные из `ip_source = ['https://...','https://...']` сохраняются в `newip.tmp`
- Файлы, загруженные из `community_domain_sources = ['https://...','https://...']` сохраняются в `community.tmp`

Если в файле `new.tmp` есть изменения по сравнению с `new.txt` производится ротация:
- `old.txt` удаляется из системы
- `new.txt` переименовывается в `old.txt`
- `new.tmp` переименовывается в `new.txt`

Если в файле `newip.tmp` есть изменения по сравнению с `newip.txt` производится ротация:
- `oldip.txt` удаляется из системы
- `newip.txt` переименовывается в `oldip.txt`
- `newip.tmp` переименовывается в `newip.txt`

Если в файле `community.tmp` есть изменения по сравнению с `community.txt` производится ротация:
- `community.txt` удаляется из системы
- `community.tmp` переименовывается в `community.txt`

По окончанию процесса все временные файлы загрузок и сортировки автоматически удаляются

### Файлы сохраняемые ядром в папке данных
- `new.txt` - Файл заблокированых доменов с самыми последними изменениями
- `old.txt` - Файл заблокированых доменов предыдущей версии
- `newip.txt` - Файл заблокированых IP адресов с самыми последними изменениями
- `oldip.txt` - Файл заблокированых IP адресов предыдущей версии
- `community.txt` - Файл заблокированых доменов по версии сообщества с самыми последними изменениями
- Директория `debug` с файлами внутри создаётся при установленом [флаге](#флаги) `--dumpevent`
...
01:02:54 INF Extension successfuly validated name="File Logger"
```
...
Эта секция это полный разбор [развёрнутого примера JSON события которое будет получать ваш плагин](https://github.com/SHULKERPLAY/Zapretyan-Go/blob/main/core/event_example.json).

Ниже представлена версия протокола Запретян `1`, актуальная для `v2.0.0.0+`.

- `"Ver"` - Версия JSON протокола Запретян. В рамках одной конкретной версии структура JSON не меняется. Можно использовать для проверки совместимости вашего плагина с ядром, на случай если этот протокол получит обновление. Текущая версия протокола: `1`
- `"type"` - Может содержать только два значения: `"rkn"` в случае если это событие сравнения с полем `"diff"`, `"cmd"` в случае если это команда для завершения работы плагина или для отправки конфигурации плагину
- `"kill"` - При `true` ваш плагин обязан завершить работу самостоятельно. Если не сделать этого вовремя, ядро завершит процесс плагина принудительно. В остальных случаях это поле будет `false`.
- `"path"` - Полный путь к папке с данными, настроенной в конфиге ядра. Для Linux поле будет иметь такой формат: `"/usr/bin/zapretyan-go/data"`, для Windows: `"С:\\Zapretyan-Go\\data"`
- `"cfg"` - Содержит объект с конфигурацией вашего плагина из `config.toml`. Пример того, как данные переносится из конфига можно увидеть в графе "[Как парсить события из полученного JSON](#как-парсить-события-из-полученного-json)"
- `"diff"` - Содержит объекты со всеми данными о разнице с момента прошлого сканирования
  - `"banned"` - Содержит объект с данными о заблокированых доменах с момента прошлого сканирования
    - `"empty"` - Если изменений нет, поле будет `true`. Иначе `false`.
    - `"length"` - Количество заблокированых доменов (Фактически длинна массива `"data"`). Если массив с изменениями пустой, это поле будет содержать `0`.
    - `"data"` - Массив со всеми заблокироваными доменами с прошлого сканирования. Массив формата `['domain1.com','domain2.com']`. Если изменений не было, поле будет содержать `[]`
    - `"total"` - Общее количество заблокированых доменов (Фактически количество строк файла с доменами (`new.txt`)). Если файла не существует, это поле будет содержать `0`.
  - `"unbanned"` - Содержит объект с данными о разблокированых доменах с момента прошлого сканирования
    - `"empty"` - Если изменений нет, поле будет `true`. Иначе `false`.
    - `"length"` - Количество разблокированых доменов с прошлого сканирования (Фактически длинна массива `"data"`). Если массив с изменениями пустой, это поле будет содержать `0`.
    - `"data"` - Массив со всеми разблокироваными доменами с прошлого сканирования. Массив формата `['domain1.com','domain2.com']`. Если изменений не было, поле будет содержать `[]`
  - `"banned_ip"` - Содержит объект с данными о заблокированых IP адресах с момента прошлого сканирования
    - `"empty"` - Если изменений нет, поле будет `true`. Иначе `false`.
    - `"length"` - Количество заблокированых IP адресов (Фактически длинна массива `"data"`). Если массив с изменениями пустой, это поле будет содержать `0`.
    - `"data"` - Массив со всеми заблокироваными IP адресами с прошлого сканирования. Массив формата `['123.12.1.3','225.14.21.6']`. Если изменений не было, поле будет содержать `[]`
    - `"total"` - Общее количество заблокированых IP адресов (Фактически количество строк файла с IP адресами (`newip.txt`)). Если файла не существует, это поле будет содержать `0`.
  - `"unbanned_ip"` - Содержит объект с данными о разблокированых IP адресах с момента прошлого сканирования
    - `"empty"` - Если изменений нет, поле будет `true`. Иначе `false`.
    - `"length"` - Количество разблокированых IP адресов с прошлого сканирования (Фактически длинна массива `"data"`). Если массив с изменениями пустой, это поле будет содержать `0`.
    - `"data"` - Массив со всеми разблокироваными IP адресами с прошлого сканирования. Массив формата `['123.12.1.3','225.14.21.6']`. Если изменений не было, поле будет содержать `[]`

# Прочее
- [Страница Zapretyan-Go на нашем сайте](https://lunarcreators.ru/zapretyan-go)
- [Страница Discord приложения Запретян на нашем сайте](https://lunarcreators.ru/zapretyan/app)
...
## ![](https://github.com/SHULKERPLAY/Zapretyan-Go/blob/main/core/winres/ico32.png)Собери свою Запретян
...
- Проверить установленную версию можно командой: `go version`
...
#### Вариант 1: Загрузка актуального кода (Dev)

Этот вариант содержит самые последние изменения и исправления, но может быть оттестирован недостаточно тщательно.

- **[Скачайте zip архив напрямую](https://github.com/SHULKERPLAY/Zapretyan-Go/archive/refs/heads/main.zip)**
- **Через консоль (Linux):**
```bash
wget https://github.com/SHULKERPLAY/Zapretyan-Go/archive/refs/heads/main.zip
unzip main.zip
cd Zapretyan-Go-main
```

#### Вариант 2: Клонирование репозитория через Git (Альтернатива)

Если у вас установлен `Git`, вы можете клонировать репозиторий:
...
#### Вариант 3: Загрузка последнего стабильного релиза

Если вам нужна проверенная и стабильная версия, вы можете скачать исходный код последнего релиза с помощью архивной ссылки GitHub:

* **Ссылка на архив:** `https://github.com/SHULKERPLAY/Zapretyan-Go/archive/refs/tags/LATEST_RELEASE.zip` *(замените LATEST_RELEASE на тег актуальной версии, например v2.1.1.0)*.

### Шаг 2. Сборка основного приложения (Ядро)
...
1. Перейдите в каталог с точкой входа приложения:
```bash
cd core/cmd
```
2. Выполните команду сборки в зависимости от вашей операционной системы:
- **Для Linux:**
```bash
go build -v -trimpath -ldflags="-s -w -buildid=" -o zapretyanGo .
```
- **Для Windows (CMD / PowerShell):**
```bash
go build -v -trimpath -ldflags="-s -w -buildid=" -o zapretyanGo.exe .
```
3. **Важно:** Для работы приложения рядом с полученным файлом `zapretyanGo`(`.exe`) обязательно должен находиться файл конфигурации. Скопируйте его из корневой папки `/core`:
- Перенесите файл `config.toml` в ту же директорию, где теперь лежит ваш скомпилированный файл.

### Шаг 3. Сборка расширений (Extensions)
...
```bash
cd ../.. # Возвращаемся в корень папки репозитория
```

Структура каталогов расширений может отличаться, поэтому процесс сборки делится на два типа:

#### Вариант 1: Файл `main.go` находится в корне расширения

Если внутри папки расширения (например, `/extensions/extension1`) сразу лежит файл `main.go`:

1. Перейдите в папку расширения:
```bash
cd ./extensions/extension1 # Переходим к папке расширения
```
2. Скомпилируйте его прямо на месте:
* **Linux:** `go build -v -trimpath -ldflags="-s -w -buildid=" -o extension1 .`
* **Windows:** `go build -v -trimpath -ldflags="-s -w -buildid=" -o extension1.exe .`

#### Вариант 2: Расширение содержит подпапку `cmd`
Если внутри папки расширения находится каталог `cmd` (путь вида `/extensions/extension2/cmd`):
1. Перейдите в подпапку `cmd` этого расширения из корня папки Zapretyan-Go:
```bash
cd ./extensions/extension2/cmd # Переходим к папке расширения
```
2. Выполните компиляцию:
* **Linux:** `go build -v -trimpath -ldflags="-s -w -buildid=" -o extension2 .`
* **Windows:** `go build -v -trimpath -ldflags="-s -w -buildid=" -o extension2.exe .`
...
### Итоговая структура рабочей папки
